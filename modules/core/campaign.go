...
func campaignRoutes(router *gin.Engine) {
	campaign := router.Group("/api/campaign")
	{
		...
		campaign.POST("/assign/:campaign/creative", assignCreativeCampaignHandler)
...
// assignAdUnitCampaignHandler adds adunit to the specified campaign
func assignCreativeCampaignHandler(c *gin.Context) {
	...
	type Creative struct {
		Creative string `json:"creative" binding:"required"`
	}
	var creative Creative
	bindErr := c.ShouldBindJSON(&creative)
	if bindErr != nil {
		log.Println(bindErr)
		c.JSON(http.StatusBadRequest, gin.H{
			"error": true,
	...
	creativeKey, _ := as.NewKey(config.Conf.Database.Aerospike.Namespace, "creatives", creative.Creative)
	creativeExists, creativeErr := master.Client.Exists(master.Client.DefaultPolicy, creativeKey)
	if creativeErr != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": true,
			"msg":   creativeErr.Error(),
		})
		return
	}
	if !creativeExists {
	...
	// Add creative to the campaign
	listPolicy := as.NewListPolicy(as.ListOrderOrdered, as.ListWriteFlagsAddUnique)
	appendCreative := as.ListAppendWithPolicyOp(listPolicy, "creatives", creative.Creative)
	records, err := master.Client.Operate(master.Client.DefaultWritePolicy, campaignKey, appendCreative)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": true,
			"msg":   err.Error(),
	...
	c.JSON(http.StatusCreated, gin.H{
		"msg": records.Bins["creatives"].(int),
	})
}
...
func getAllCampaignsHandler(c *gin.Context) {
	...
	stmt := as.NewStatement(config.Conf.Database.Aerospike.Namespace, "campaigns", "name", "description", "advertiser", "creatives", "adunits", "status", "startAt", "endAt")
	...
	for res := range rs.Results() {
		if res.Err != nil {
			...
			item["creatives"] = res.Record.Bins["creatives"].([]interface{})
